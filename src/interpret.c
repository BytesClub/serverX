...
\"%s\" REQUEST_METHOD=\"%s\" exec /usr/bin/php-cgi ";
    int command_len = strlen(format) + strlen(query) + strlen(path) + strlen(method) - 4;
    char command[command_len];
    if (snprintf(command, command_len, format, query, path, method) < 0) {
        error(cfd, 500);
        return;
    }
